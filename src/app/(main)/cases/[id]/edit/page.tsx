...
import type { Category, CaseDetail } from '@/types';
...
export default function EditCasePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/cases/${params.id}`).then(r => r.json()),
      fetch('/api/categories').then(r => r.json()),
    ]).then(([caseData, catData]) => {
      if (!caseData.success) { setError(caseData.error); return; }
      const c = caseData.data as CaseDetail;
      if (c.author.id !== user.id) { setError('无权编辑'); return; }
      setTitle(c.title);
      setContent(c.content);
      setCategoryId(c.category.id.toString());
      if (c.images?.length) setImages(c.images.map((img: { id: number; url: string }) => ({ id: img.id, url: img.url })));
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.success) {
        setImages(prev => [...prev, { id: data.data.id, url: data.data.url }]);
        toast.success('上传成功');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/cases/${params.id}`, {
        ...
        body: JSON.stringify({ title, content, categoryId: parseInt(categoryId), imageIds: images.map(img => img.id) }),
      });
      const data = await res.json();
      if (data.success) {
        toast.success('已保存');
  ...
  if (loading || authLoading) return <div className="text-center py-20 text-gray-400">加载中...</div>;
  if (error) return <div className="text-center py-20 text-gray-400">{error}</div>;

  return (
    <div className="max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">编辑案例</h1>
